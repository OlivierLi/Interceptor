...
void Player::update(const World &world){
    ...
    for(sf::Event &event:*world.frame_events){
        if(event.type == sf::Event::MouseMoved){
            last_known_positions.push_back(std::pair<int,int>(event.mouseMove.x,SCREEN_RESOLUTION_Y - event.mouseMove.y));
        }
    }
...
//This function returns shortest distance to one of the last know locations of the player
